...
| 2019年7月18日 | 进展到沟通流程，先让所有人同步目标，沟通方向  |
...
- [开发板介绍（已审，存在的问题）](https://demo.codimd.org/XCESTmS6Tw-nCKtt-IGu1Q)
   - BIT 各类元件描述图，光发提供一个 PSD 工程。
   - 缺少 尺寸规格图。
   - 是否觉得需要修订 正面与背面 的效果图，待渲染。
   - XXX 设计一张 两者整合 的图，图中带 LED 编号，待处理。
   - 底部金手指定义 的 原始表格 ，限定为 1.4 版本，已建立，待校正。

### 2019年7月18日

- [什么是 Web:Bit 教育版（已审，存在的问题）](https://demo.codimd.org/jJ2G6dd6Sf63MiWBmMIJFw)
   - 替换效果图，整体预览，~~放购买链接二维码图之类的~~。
   - 缺少 3D 效果图，待处理。
   - 编辑器的电脑预览图简体化，待处理。
   - 修复内部的链接或下载链接，待修复。
